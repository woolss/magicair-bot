...
function isOrderMessage(text) {
  ...
  // Если это FAQ вопрос, то НЕ заказ
  ...
  // Считаем заказом только если есть и действие, и товар
  // ИЛИ если есть явное упоминание заказа/покупки
...
function isOrderContext(chatId) {
  ...
  // Проверяем, был ли недавно заказ (в течение 10 минут)
...
function isOrderClarification(text, chatId) {
  ...
  // Если клиент недавно делал заказ
  if (!isOrderContext(chatId)) return false;
  
  // Ключевые слова уточнений
...
// 🔎 Вопросы про цену обрабатываются в FAQ, не считаем их уточнением заказа
...
// Считаем заказ детализированным только если есть хотя бы 2 из 4 параметров
...
if (!hasEnoughDetails) {
  // ❌ Недостаточно деталей → задаём уточняющие вопросы
  let clarificationMessage = "Для оформлення замовлення, будь ласка, уточніть:\n\n";
  
  if (!hasQuantity) {
    clarificationMessage += "📦 Скільки кульок потрібно?\n";
  }
  if (!hasSpecificType) {
    clarificationMessage += "🎈 Які саме кульки: латексні, фольговані, цифри?\n";
  }
  if (!hasDate) {
    clarificationMessage += "📅 На коли потрібна доставка?\n";
  }
  if (!hasStore) {
    clarificationMessage += "📍 Доставка чи самовивіз (з якого магазину)?\n";
  }
...
// ✅ Есть достаточно деталей → можно уведомлять менеджеров
let clientResponse;
if (isDirectOrder) {
...
14. **КРИТИЧНО ВАЖЛИВО - РОЗПІЗНАВАННЯ ЗАМОВЛЕНЬ:**
    - Якщо клієнт пише будь-що схоже на замовлення або уточнення до замовлення, ЗАВЖДИ відповідай: "Передаю ваше замовлення/уточнення менеджеру для підтвердження..."
    - Замовлення це будь-яке повідомлення з: назвою товару + кількістю, часом доставки, місцем отримання, уточненнями типу товару  
    - НІКОЛИ не давай поради по замовленню - одразу передавай менеджеру!
</rules>
