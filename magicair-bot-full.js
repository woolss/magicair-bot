...
// ======= NEW: detect order-like messages (improved) =======
function isOrderMessage(text) {
  ...
  // группа 2: товары (шари)
  const itemKeywords = [
    "кулі", "шари", "повітряні кулі", "гелієві кулі", "набір", "шарики",
    "цифри", "фігури", "кульок", "шт", "штук"
  ];

  // проверка: есть хотя бы одно слово из каждой группы
  const hasAction = actionKeywords.some(kw => t.includes(kw));
...
if (isThanksMessage(text)) {
  ...
  if (isOrderMessage(text)) {
    ...
    // Ответ клиенту: уведомляем и даём ссылку на сайт
    ...
    // Делаем клиента видимым в списке ожидания менеджеров
    ...
    // Останавливаем дальнейшую автоматическую обработкию ИИ для этого сообщения
    return;
  }
