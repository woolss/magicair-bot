...
// ======= Покращена логіка розпізнавання замовлень =======
function isOrderMessage(text) {
  if (!text) return false;
  const t = text.toLowerCase().trim();

  // 🧠 Якщо користувач питає «де/як купити чи замовити» — це не замовлення, а запит
  if (/^(де|як)\s+(купити|замовити|придбати)/.test(t)) return false;
  if (/(де|як)\s+(можна\s+)?(купити|замовити|придбати)/.test(t)) return false;
  if (/(где|как)\s+(купить|заказать)/.test(t)) return false;
  if (/(можно|де)\s+(купити|замовити|заказать)\s*(тут|у вас)?/.test(t)) return false;

  const directOrderKeywords = [
    "замовити", "замовлення", "замовлю", "закажу", "замовляю",
    "купити", "придбати", "візьму", "беру", "потрібно", "потрібні",
    "доставка", "доставку", "привезіть", "привезти", "привезіть мені",
    "можна доставку", "можна замовити", "оформити", "оформлення",
    "хочу замовити", "хочу заказати", "хочу заказ", "замовимо",
    "нужны шарики", "нужны кульки", "заказать", "закажіть"
  ];

  const itemKeywords = [
    "кулі", "кульки", "кульок", "шари", "шарики",
    "повітряні кулі", "гелієві кулі", "гелій", "набір", "набор",
    "цифри", "цифра", "цифру", "фігури", "фігура",
    "ходячі", "ходяча", "латексні", "фольговані", "однотонні",
    "з малюнком", "з конфеті", "агат", "браш", "з бантиками",
    "серце", "серця", "зірка", "зірки", "букви", "баблс", "bubble", "надувні"
  ];

  const faqQuestions = [
    "скільки коштує", "ціна", "яка ціна", "які є", "які бувають",
    "показати варіанти", "каталог", "асортимент", "фото", "зображення",
    "як оплатити", "оплата", "чи є доставка", "самовивіз",
    "де знаходитесь", "адреса", "гарантія", "з чого зроблені",
    "чи безпечні", "як довго тримаються", "години роботи", "коли працюєте"
  ];

  // 🟡 Якщо це запит з FAQ — теж не замовлення
  ...
  const hasQuantityAndItem = /\d+\s*(штук|шт|кульок|кулі|шарів|шарики|цифри|фігура|фігури)/i.test(t);

  if (hasDirectAction && hasItem) return true;
  if (hasQuantityAndItem) return true;
  if (/(доставка|привезіть|самовивіз)/.test(t) && hasItem) return true;
  if ((t.includes("завтра") || t.includes("сьогодні") || t.includes("післязавтра")) && hasItem) return true;

  return false;
}

// ======= Перевірка повноти замовлення =======
function isCompleteOrder(text) {
  if (!text) return false;
  const t = text.toLowerCase();

  const hasQuantity = /\d+/.test(t) || /(шт|штук)/.test(t);
  const hasType = /(латексні|фольговані|цифри|фігури|різнокольорові|однотонні)/.test(t);
...
function isOrderClarification(text, chatId) {
  ...
  const clarificationKeywords = [
    "латексні", "фольговані", "однотонні", "з малюнком", "з конфеті",
    "агат", "браш", "з бантиками", "цифри", "фігури", "серця", "зірки",
    "ходячі", "теремки", "оболонь", "самовивіз", "доставка", "сьогодні", "завтра"
  ];
  return clarificationKeywords.some(kw => t.includes(kw));
}
...
function isThanksMessage(text) {
  if (!text) return false;
  const t = text.toLowerCase();
  const thanksKeywords = ["дякую", "спасибі", "вдячний", "дякую вам", "спасибо", "благодарю"];
...
async function handlePhotoMessage(msg) {
  ...
  if (profile.orderStatus === 'collecting') {
    ...
    profile.lastOrder += `\n[Фото уточнення] ${caption || "(без коментаря)"}`;
    await bot.sendMessage(chatId, "📸 Додано фото-уточнення до замовлення.", orderCollectionMenu);
    return;
  }

  // --- НОВЕ фото-замовлення ---
  profile.orderType = 'photo';
  profile.orderStatus = 'collecting';
  profile.lastClarified = false;
  profile.lastPhotoOrder = { fileId, caption };
  profile.lastOrder = caption || "(фото без коментаря)";
  profile.lastOrderTime = Date.now();

  if (!caption) {
    await bot.sendMessage(chatId,
      "📷 Ви надіслали фото кульок. Уточніть, будь ласка:\n📅 На коли потрібна доставка?\n📍 Доставка чи самовивіз?",
      orderCollectionMenu
    );
  } else {
    await bot.sendMessage(chatId,
      "✅ Ваше фото-замовлення готове до відправки!\n🎯 Натисніть '✅ Відправити замовлення менеджеру'.",
      orderCollectionMenu
    );
  }
...
async function finalizeAndSendOrder(chatId, userName) {
  ...
  // 🚫 Якщо вже відправлено — нічого не робимо
  if (profile.orderStatus === 'sent') return;

  // 🚫 Блокуємо подальші уточнення
  profile.orderLocked = true;

  // 🧹 Очищаємо таймер автопідтвердження (якщо був)
  if (profile.autoSendTimer) {
    clearTimeout(profile.autoSendTimer);
    delete profile.autoSendTimer;
  }

  // 🟢 Позначаємо замовлення як відправлене
  ...
  if (profile.orderType === "photo" && profile.lastPhotoOrder) {
    for (const managerId of notifyList) {
      try {
        ...
        console.error("❌ Failed to notify manager with photo order:", err.message);
      }
    }
  } else {
    for (const managerId of notifyList) {
      try {
        ...
        console.error("❌ Failed to notify manager with text order:", err.message);
      }
    }
  }
...
// 🏠 Якщо користувач натиснув "Головне меню" — виходимо з чату або повертаємось у меню
if (text === "🏠 Головне меню") {
  if (userStates[chatId]?.step === 'manager_chat') {
    // 🧩 Якщо клієнт у чаті з менеджером — завершуємо чат
    await handleEndCommand(chatId);
  } else {
    // 🧹 Якщо клієнт не в чаті — просто очищаємо стан замовлення
...
if (userProfiles[chatId]?.orderLocked || userProfiles[chatId]?.orderStatus === 'ready') {
  ...
  // 3️⃣ Обробка кнопок меню
  const menuHandled = await handleMenuActions(chatId, text, userName);
  if (menuHandled) return;

  // 4️⃣ Ініціалізація профілю
  if (!userProfiles[chatId]) {
    userProfiles[chatId] = { 
      chatId, 
      created: Date.now(), 
...
async function handleMenuActions(chatId, text, userName) {
  ...
  return true;
    ...
    // 🔥 Обробка кнопки відправки замовлення
  ...
  // Повертаємо false, якщо текст не є командою меню
  return false;
}
    ...
    switch (data) {
      ...
      case 'foil_figures':
        await sendProductInfo(chatId, messageId,
          ...
          'Фігурні кулі: тварини, персонажі, з написом.',
...
4. **Графік роботи:** 
   - Якщо клієнт запитує про години роботи магазинів, завжди відповідай окремо:
     * Магазин **Теремки** працює 24/7.
     * Магазин **Оболонь** працює з **09:00 до 19:00**.
   - Якщо запитують про **доставку**, зазначай, що доставка працює 24/7,
     але магазини мають свій графік роботи (видача замовлень або самовивіз може бути обмежений годинами роботи магазину).
   - Якщо клієнт запитує «чи можна відправити з Оболоні» або «з Теремків» — 
     відповідай правдиво, що з **Оболоні можна відправити лише в робочий час 09:00–19:00**, 
     а з **Теремків — цілодобово**.
...
13. **Заборона прийому або імітації оформлення замовлень:**  
   - Ти **ніколи** не приймаєш, не оформлюєш і не підтверджуєш замовлення клієнта.  
   - Навіть якщо клієнт пише «підтверджую», «оформлюйте», «замовляю», «зробіть доставку»,  
     **не відповідай так, ніби замовлення вже прийнято або буде виконане.**  
   - Замість цього відповідай коротко і ввічливо:  
     _"Дякую за уточнення! Щоб оформити замовлення, будь ласка, натисніть кнопку **«💬 Менеджер»** у меню або оформіть самостійно на сайті [magicair.com.ua](https://magicair.com.ua)."_  
   - Якщо клієнт запитує, **де можна оформити замовлення**, **чи можна замовити прямо тут**, **як купити**,  
     або пише подібні питання російською мовою, наприклад: **где оформить заказ**, **как сделать заказ**, **можно заказать тут**, **как купить**,  
     завжди пояснюй, що є **3 зручні способи**:
     1️⃣ **Надіслати фото кульок прямо в чат** — це допоможе менеджеру швидше зрозуміти, які саме кульки потрібні.  
     2️⃣ Натиснути кнопку **«💬 Менеджер»** у меню, щоб менеджер допоміг уточнити деталі.  
     3️⃣ Або оформити самостійно на сайті [magicair.com.ua](https://magicair.com.ua) — там можна одразу вибрати кульки і оплатити онлайн.  
+  - Якщо клієнт запитує **чи можна купити у вашому магазині**, або формулює схожі питання, наприклад:  
+    **«де можна купити кульки», «купити у магазині», «чи можна купити на місці», «чи є продаж у магазині»,**  
+    або російською: **«где можно купить шары», «покупка в магазине», «можно прийти купить», «у вас есть продажа в магазине»**,  
+    тоді завжди уточни, **в якому саме магазині — Теремки чи Оболонь — планується покупка**,  
+    і повідом, що:  
+    * на **Теремках** видача працює **24/7**,  
+    * на **Оболоні** — **з 09:00 до 19:00**.  
   - Не пропонуй оформити або підтвердити замовлення через чат.  
   - Ти лише консультант і не береш участі в процесі замовлення, доставлення або оплати.
14. **AI — це навігатор.**
    Якщо питання клієнта напряму стосується однієї з категорій FAQ  
    (**Доставка та оплата**, **Про кулі та гелій**, **Замовлення та терміни**,  
    **Оформлення та декор**, **Контакти та режим роботи**),  
    спочатку дай коротку відповідь, використовуючи дані з <data>.  
    Потім *обов’язково* запропонуй перейти до відповідного розділу FAQ-меню для детальної інформації  
    (наприклад: "Більше про доставку та оплату можна переглянути у розділі «🚚 Доставка та оплата» нижче 👇").  
    Якщо запит комбінований (наприклад, доставка + терміни) — коротко згадай обидва пункти й запропонуй обидва переходи.
</rules>
...
* **Фольговані цифри з гелієм:** від 385 до 590 грн.  
  Доступні розміри: **70 см** і **100 см**.  
  - **70 см:** лише у кольорах **синій, срібний, фуксія, золотий**.  
  - **100 см:** у кольорах **срібний, золотий, рожевий, синій, червоний, чорний, райдужний, рожеве золото, блакитний, кремовий, фуксія**.
...
* **Наші магазини:**
    * **Теремки:** вул. Метрологічна 13. Видача замовлень і доставка працюють 24/7.  
        * Телефон: (063) 233-33-03
    * **Оболонь:** вул. Героїв полку Азов 24/10. Графік роботи: 09:00–19:00.  
        * Усі замовлення з Оболоні (доставка або самовивіз) можливі лише в цей час.  
        * Телефон: (095) 634-63-64
* **Доставка:** Працює 24/7 по Києву та області.  
   Якщо відправлення здійснюється з **Теремків** — без обмежень у часі.  
   Якщо з **Оболоні** — лише в її робочі години (09:00–19:00).  
   Вартість розраховується за тарифами таксі.
* **Тривалість польоту:**
...
* **Контакти:**
    * **Теремки:** вул. Метрологічна 13  
        📍 [Відкрити на Google Maps](https://maps.app.goo.gl/3ssoS3bBcN3kur2r8)  
        Телефон: (063) 233-33-03  
    * **Оболонь:** вул. Героїв полку Азов 24/10  
        📍 [Відкрити на Google Maps](https://maps.app.goo.gl/1xerEqqqWpwVPoJQ6)  
        Телефон: (095) 634-63-64  
    * Сайт: [magicair.com.ua](https://magicair.com.ua)
...
📱 *Соцмережі:*  
• Instagram: [magic_air.kiev](https://instagram.com/magic_air.kiev)
